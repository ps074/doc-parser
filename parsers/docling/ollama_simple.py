#!/usr/bin/env python3
"""Docling + Ollama (simple prompt) parser."""
import argparse
import time
...
PICTURE_OPTIONS = PictureDescriptionApiOptions(
    ...
    params=dict[str, Any](model="qwen3-vl:2b", seed=42, max_completion_tokens=128),
    prompt="Describe this image briefly.",
    timeout=600
)

def parse_document(pdf_path: Path) -> str:
    """Parse PDF with Ollama (simple prompt) image descriptions."""
...
def main():
    parser = argparse.ArgumentParser(description="Parse PDFs with Docling + Ollama (simple)")
    ...
    output_dir = Path("output/docling/ollama-simple") / input_path.stem
    output_dir.mkdir(parents=True, exist_ok=True)
    output_path = Path(args.output) if args.output else output_dir / f"{input_path.stem}.md"

    print(f"Parsing: {input_path.name} (VLM: Ollama qwen3-vl:2b - simple prompt)")
    ...
    perf_header = f"""# Docling + Ollama (Simple) Performance

**Document:** {input_path.name}
**VLM Backend:** Ollama qwen3-vl:2b
**Prompt:** Simple ("Describe this image briefly.")
**Max Tokens:** 128
